...
export function exportDataFlowGraphHtml(f: FragmentState, filename: string) {
    ...
    exportPromiseDataFlowJson(f,filename+".json");
}
...
type PromiseDataFlow = {
    paths: Array<PromiseFlow[]>;
}

function getPromiseDataFlows(f: FragmentState):  Map<String, PromiseDataFlow>  {
    const promiseFlows = new Map<String, PromiseDataFlow>();
    
    function trackPromiseFlow(node: NodeVar): void {
        const paths = findAllPaths(node);
        if (paths.length > 0) {
            promiseFlows.set(getNodeIdentifier(node), { paths });
        }
    }
    
    function findAllPaths(startNode: NodeVar): Array<PromiseFlow[]> {
...
function getNodeIdentifier(node: NodeVar): string {
    // Get the hash using solver's getNodeHash method
    const hash = Solver.prototype.getNodeHash(node.node);
...
/**
 * Exports the promise dataflow information as a JSON file.
 */
export function exportPromiseDataFlowJson(f: FragmentState, filename: string) {
    const promiseFlows = getPromiseDataFlows(f);
    writeFileSync(filename, JSON.stringify(Object.fromEntries(promiseFlows), null, 2));
}
